...
def index():
    items = [
        {'label': 'Course Catalog', 'path': plugin.url_for('course_catalog')},
        {'label': 'My Courses (not implemented yet)', 'path': plugin.url_for('my_courses')}
    ]
...
def play_lecture(lec_id):
    youtube_url = 'plugin://plugin.video.youtube/?action=play_video&videoid={0}'.format(lec_id)
